...
### 입출력 정의
- **입력**: 사용자가 등록한 키워드(문자열), 네이버 뉴스 검색 API 응답(JSON), 사용자의 GitHub PAT(브라우저에만 저장)
...
- "업데이트"/"키워드 추가"는 **본인의 GitHub Personal Access Token**(fine-grained, 이 저장소의 Actions·Contents 쓰기 권한만)이 브라우저 localStorage에 저장되어 있어야 동작. 토큰이 없는 방문자는 읽기만 가능
...
- PAT는 fine-grained 토큰으로 발급하고 권한을 이 저장소의 `Contents: Read/Write`, `Actions: Read/Write`로만 제한한다.
